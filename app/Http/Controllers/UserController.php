      ...
      if($request->has('password')){
        ...
        if(Hash::check($request->current_password, $user->password) && $request->password == $request->password_confirmation){   
          //dd('benar');       
            ...
            return redirect()->back()->with('success','New Password has been applied.');        
        } else {
          return redirect()->back()->with('change-password-error','To change password, you must input the correct data according to the instruction.');
        }
      }else{
